...
str(pp)
summary(pp)
pp <- pp[,-4]  #retirando a coluna valor
```
...
kNN é sensivel a distribuição dos dados por variável/atributo/features, pois as distâncias são calculadas em relação a escala de cada eixo. 
...
confusionMatrix(knnPredict, testing$classes )
